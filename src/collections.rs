...
impl Collection {
    ...
    pub fn load_objects(conn: &rusqlite::Connection, entities: &HashMap<CoId, String>) -> Vec<Collection> {
        let mut collections: Vec<Collection> = vec![];

        if let Ok(mut stmt) = conn.prepare("SELECT Z_ENT, Z_PK, ZNAME, ZPARENT, ZFOLDERLOCATION FROM ZCOLLECTION") {
            ...
            while let Some(Ok(row)) = rows.next() {
                ...
                if let Some(entity_name) = entities.get(&entity) {
                    ...
                    let collection_type = match entity_name.as_str() {
                        ...
                        "CatalogAllImagesCollection" =>
                            CollectionType::CatalogAll,
                        "CatalogInternalImagesCollection" =>
                            CollectionType::CatalogInternalImages,
                        "TrashCollection" => CollectionType::Trash,
                        "AlbumCollection" => CollectionType::Album(row.get(2)),
                        "CatalogFolderCollection" =>
                            CollectionType::Folder(row.get(4)),
                        "VirtualFolderCollection" =>
                            CollectionType::VirtualFolder(row.get(2)),
                        _ => {
                            println!("Unhandled entity {}", entity_name.as_str());
                            continue;
                        },
                    };
                    collections.push(Collection { id, collection_type, parent });
                }
            }
        }
